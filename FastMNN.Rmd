...
# Functions needed outside the huge fastMNN function
...
```{r}
# Centering and scaling function for PCA
...
```{r}
# one of the PCA functions needed for fastMNN
...
{
    batches <- .unpackLists(...) # unpacking lists allows for extraction of individual values (the batch values here)
    checkBatchConsistency(batches, cells.in.columns=TRUE) # checking for identical number of rows (and rownames) 
    restrict <- checkRestrictions(batches, restrict, cells.in.columns=TRUE) # identifying which cells to use in the batch correction?

    # Extracting information from SCEs. <- SCEs being "single cell expressions"
    is.sce <- checkIfSCE(batches)
    if (any(is.sce)) {
        # NOTE: do NOT set withDimnames=FALSE, this will break the consistency check.
        batches[is.sce] <- lapply(batches[is.sce], assay, i=assay.type)  # applying the function assay over the list of batch values (?)
    }

    # Setting up the parallelization environment. (between the PCA and the nearest-neighbors search)
    if (.bpNotSharedOrUp(BPPARAM)) {
        bpstart(BPPARAM)
        on.exit(bpstop(BPPARAM), add=TRUE)
    }

    BSPARAM <- .set_deferred(BSPARAM, deferred) # wait on centering and scaling, BSPARAM specifies the algorithm to use for PCA
    ...
    if (length(batches)==1L) {  # in the case that there is only one batch
        if (is.null(batch)) { 
            stop("'batch' must be specified if '...' has only one object")
        }
    ...
    if (!is.null(subset.row) && !correct.all) {  # if the features that will be used for batch correction are not specified or if corrected expression values shouldn't be computed for features that won't be used for batch correction, then take subset of whole list of batch values and keep the batch values for only the features that will be used for batch correction
    ...
    correct.all=FALSE, BSPARAM=ExactParam(), BPPARAM=SerialParam()) 
{
    ...
    if (cos.norm) { 
        ...
        batch.list <- mapply(FUN=.apply_cosine_norm, batch.list, all.l2s, SIMPLIFY=FALSE) # do cosine normalization to multiple list or vector arguments
    }

    if (!is.na(d)) { # if a value for the # dimensions to use for dimensionality reduction is specified
        ...
        pc.mat <- lapply(batch.list, t) # transpose the cosine normalized list/vector arguments
        pc.mat <- lapply(pc.mat, as.matrix) # make the transposed output a matrix
        pc.mat <- List(pc.mat) # make the matrix a list (?)
    }

    out <- .fast_mnn(batches=pc.mat, ..., BPPARAM=BPPARAM)
    .convert_to_SCE(out, pc.mat) # convert mutual nearest neighbors to single cell expression data using the centered/scaled cosine normalized list/vector arguments (?)
}
    ...
    correct.all=FALSE, BSPARAM=ExactParam(), BPPARAM=SerialParam()) 
{
    ...
    if (cos.norm) { 
        l2 <- cosineNorm(x, mode="l2norm", subset.row=subset.row, BPPARAM=BPPARAM) # a double precision vector is returned containing the L2 norm for each cell
        x <- .apply_cosine_norm(x, l2) # do cosine normalization
    }

    if (!is.na(d)) { # if the # dimensions to be used for dimensionality reduction are specified, do the centering/scaling for PCA
    ...
    divided <- divideIntoBatches(mat[[1]], batch=batch, restrict=restrict, byrow=TRUE) # Divide a single input object into multiple separate objects according to their batch of origin.
    ...
    #merge.info --> a dataframe in metadata where each row corresponds to a merge step
    #.reindex_pairings restores the MNN pairing indices to the original positions (i.e., after cells have been reordered by new.order)
...
# A "wrapper" wraps, i.e. calls, another function that does the real work but provides a different or more convenient (or more convenient for a specific purpose) interface to it or specific syntax.
...
{
    if (!auto.merge) { # if merging is not set to automatic
    ...
    NEXT, UPDATE)
{
    ...
    for (mdx in seq_len(nmerges)) {
        ...
        left.set[[mdx]] <- left.index  # not sure what this is
        right.set[[mdx]] <- right.index  # not sure what this is either
        ...
        overall.batch <- colMeans(ave.out$averaged) # average of averages (?)
    ...
    if (is.unsorted(full.order)) {
        ncells.per.batch <- tabulate(full.origin) # counts the number of cells in each batch (I believe) and puts the results in a table
